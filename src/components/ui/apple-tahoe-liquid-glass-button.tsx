...
// I will paste the real base64 string here later!
...
const GlassButton = React.forwardRef<HTMLButtonElement, GlassButtonProps>(
  (
    ...
    const scopedClassName = `btn-liquid-${filterId}`;
...
function LiquidGlassFilter({
  ...
  return (
    <>
      ...
      <style>{`
        .${scopedClassName}.btn-liquid {
          appearance: none; border: none; background: transparent;
          color: oklch(from var(--foreground) l c h / 95%);
          --glass-reflex-light: 1; --glass-reflex-dark: 1;
        }
        .${scopedClassName} .btn-liquid-lens {
          ...
          box-shadow: inset 0 0 0 1px color-mix(in srgb, white calc(var(--glass-reflex-light) * 15%), transparent), inset 1.8px 3px 0px -2px color-mix(in srgb, white calc(var(--glass-reflex-light) * 45%), transparent), inset -2px -2px 0px -2px color-mix(in srgb, white calc(var(--glass-reflex-light) * 35%), transparent), inset -3px -8px 1px -6px color-mix(in srgb, white calc(var(--glass-reflex-light) * 20%), transparent), inset -0.3px -1px 4px 0px color-mix(in srgb, black calc(var(--glass-reflex-dark) * 12%), transparent), inset -1.5px 2.5px 0px -2px color-mix(in srgb, black calc(var(--glass-reflex-dark) * 20%), transparent), inset 0px 3px 4px -2px color-mix(in srgb, black calc(var(--glass-reflex-dark) * 20%), transparent), inset 2px -6.5px 1px -4px color-mix(in srgb, black calc(var(--glass-reflex-dark) * 10%), transparent), 0px 1px 5px 0px color-mix(in srgb, black calc(var(--glass-reflex-dark) * 10%), transparent), 0px 6px 16px 0px color-mix(in srgb, black calc(var(--glass-reflex-dark) * 8%), transparent);
          transition: background-color 400ms cubic-bezier(1, 0.0, 0.4, 1), box-shadow 400ms cubic-bezier(1, 0.0, 0.4, 1);
        }
        .${scopedClassName} .btn-liquid-text {
          text-shadow: 0 1px 2px oklch(from var(--background) l c h / 30%);
          transition: color 400ms cubic-bezier(1, 0.0, 0.4, 1);
        }
        @media (hover: hover) { .${scopedClassName}.btn-liquid:not(:disabled):hover { transform: scale(1.03); } }
        .${scopedClassName}.btn-liquid:not(:disabled):active { transform: scale(0.96); }
      `}</style>
    </>
  );
}
